...
<td class="py-2 px-3 md:px-6 text-left">
    ...
    >
         ...
         <tr 
            id="info-row-${index}" 
            class="bg-purple-100 border-2 border-l-gray-100 hidden transition-all duration-300 ease-in-out overflow-hidden opacity-0"
        >
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function (e) {
            ...
            showConfirmModal(index, (employeeId) => {
                deleteEmployee(employeeId, currentPage);
            });
        });
    });
    ...
    document.querySelectorAll('.info-btn').forEach(button => {
        button.addEventListener('click', function (e) {
            ...
            if (isHidden) {
                ...
                this.classList.add('bg-green-500'); // Change color when open
            } else {
                ...
                infoRow.addEventListener('transitionend', () => {
                    ...
                    this.classList.remove('bg-green-500'); // Revert color when closed
                }, { once: true });
            }
        });
    });
...
function modelEmployee(index) {
    if(index){
        ...
        .then(employee => {
            let modalContent = `
                <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70">
                    <div class="bg-white rounded-lg shadow-lg p-8 w-11/12 md:w-1/3">
                        <h2 class="text-xl font-bold mb-4 text-center text-purple-600">Edit Employee</h2>
                        <form id="editForm">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" id="editFullName" class="mt-1 text-sm block w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-purple-500" value="${employee.fullName}" required />
                            </div>
                            <div class="mb-4">
                                <label for="editEmail" class="block text-sm font-medium text-gray-700">Email</label>
                                <input type="email" id="editEmail" class="mt-1 block text-sm w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-purple-500" value="${employee.email}" required />
                            </div>
                            <div class="mb-4">
                                <label for="editJobTitle" class="block text-sm font-medium text-gray-700">Job Title</label>
                                <input type="text" id="editJobTitle" class="mt-1 block w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-purple-500" value="${employee.jobTitle}" required />
                            </div>
                            <div class="flex justify-between">
                                <button type="button" id="closeModal" class="mr-2 text-sm  w-1/3  py-2 bg-gray-300 rounded-md hover:bg-gray-400 transition">Cancel</button>
                                <button type="submit" class=" w-1/3 py-2 bg-purple-600 text-sm text-white rounded-md hover:bg-purple-700 transition">Save</button>
                            </div>
                        </form>
                    </div>
                </div>`;
        ...
        <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70">
            <div class="bg-white rounded-lg shadow-lg p-8 w-11/12 md:w-1/3">
                <h2 class="text-xl font-bold mb-4 text-center text-purple-600">Add New Employee</h2>
                <form id="editForm">
                    <input type="hidden" id="editEmployeeId" value="" />
                    <div class="mb-4">
                        <label for="editFullName" class="block text-sm font-medium text-gray-700">Full Name</label>
                        <input type="text" id="editFullName" class="mt-1 text-sm block w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-purple-500" value="" required />
                    </div>
                    <div class="mb-4">
                        <label for="editEmail" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" id="editEmail" class="mt-1 text-sm block w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-purple-500" value="" required />
                    </div>
                    <div class="mb-4">
                        <label for="editJobTitle" class="block text-sm font-medium text-gray-700">Job Title</label>
                        <input type="text" id="editJobTitle" class="mt-1 block w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-purple-500" value="" required />
                    </div>
                    <div class="flex justify-between">
                        <button type="button" id="closeModal" class="mr-2 w-1/3   py-2 bg-gray-300 rounded-md hover:bg-gray-400 transition">Cancel</button>
                        <button type="submit" class=" w-1/3  py-2 bg-purple-600 text-white text-sm rounded-md hover:bg-purple-700 transition">Add</button>
                    </div>
                </form>
            </div>
        </div>`;
...
function pagination(currentPage) {
    ...
    const buttonWidth = '100px'; // Set a fixed width for the buttons

    const prevButton = document.createElement('button');
    prevButton.className = `flex items-center justify-center px-3 py-2 bg-purple-600 text-white text-sm md:text-md rounded hover:bg-purple-700 transition duration-300 w-${buttonWidth}`;
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pageButton.className = `px-3 py-2 rounded text-sm hover:bg-purple-600 transition duration-300 ${
            currentPage === i ? 'bg-purple-700 text-white' : 'bg-gray-300 text-gray-800'
        }`;
    ...
    nextButton.className = `flex items-center justify-center px-3 py-2 bg-purple-600 text-white text-sm md:text-md rounded hover:bg-purple-700 transition duration-300 w-${buttonWidth}`;
...
function showConfirmModal(employeeId, callback) {
    const modalContent = `
        <div id="confirmModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70">
            <div class="bg-white rounded-lg shadow-2xl p-6 w-11/12 md:w-1/3">
                <h2 class="text-lg font-bold  mb-4 text-center text-red-600">Confirm </h2>
                <p class="mb-4 text-left">Are you sure you want to delete this employee?</p>
                <div class="flex justify-between">
                    <button id="cancelDelete" class="mr-2 px-4 py-2 bg-gray-300 rounded-md hover:bg-gray-400 transition">Cancel</button>
    ...
    document.getElementById('confirmDelete').addEventListener('click', function() {
        callback(employeeId); // Call the callback with the employeeId
        closeModal();
    });
